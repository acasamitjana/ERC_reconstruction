...
% not satisfactory
% This script is for the cerebellum, and is heavily based on
...
d=dir([blockFaceStackDir filesep casename '_C*.gray.nii.gz']);
f=find(d(end).name=='.');
NC=str2double(d(end).name(f(1)-1));
slices=1:NC;
...
for s=nSlices:-1:1
    ...
    disp(['Working on slice ' direction num2str(slice) ' (' num2str(nSlices-s+1) ' of ' num2str(nSlices) ')']);
    ...
    for Bind=1:nBlock
        ...
        if slices(s)~=NC
            ...
            elseif thicknessMeasured>8 && thicknessMeasured<12
                ...
                disp(['Warning: thickness is outside [3,7] and [8 12] for slice ' num2str(s) ', block ' num2str(Bind) ]);
        ...
        % Dfine vox2ras0, and incorporate translation in z (medial-lateral;
        % in the future we could rotate the header at the end so z is inferior-superior as
        % it should, rather than medial/lateral, as it is now...)
        ...
        if s==nSlices
            vox2ras0(3,4)=-size(BmriMask{Bind}.vol,3)*effectiveSectionThickness + thicknesses(nSlices);
        else
            vox2ras0(3,4)=sum(thicknesses(s+1:end));
        end
    ...
    if s==nSlices, zshift=0.8*thickness; else, zshift=BmriWarped{1}.vox2ras0(3,4)+0.8*thickness; end
    mriMosaicA=makeMosaic(BmriWarped,BmriWarpedMask,zshift,RES_C,effectiveSectionThickness);
    if s==nSlices, zshift=0.05*thickness; else, zshift=BmriWarped{1}.vox2ras0(3,4)+0.05*thickness; end
    ...
    if s==nSlices % First slice, nothing to do...
...
d=dir([outputdir filesep casename '_C*_volume.gray.initialized.mgz']);
for i=1:length(d)
...
myMRIwrite(mriResampled,[outputdir filesep 'initialResampledCerebellum.gray.nii.gz']);
mriResampled.vol=numeratorRGB./repmat(denominator,[1 1 1 3]);
myMRIwrite(mriResampled,[outputdir filesep 'initialResampledCerebellum.rgb.nii.gz']);
mriResampled.vol=255*double(denominator>0.5); 
myMRIwrite(mriResampled,[outputdir filesep 'initialResampledCerebellum.mask.nii.gz']);
