...
from config import config_dev, config_data
...
BT_DB = databaseConfig.BT_NONLINEAR[SUBJECT]
BT_DB_MRI = databaseConfig.BT_LINEAR[SUBJECT]['MRI']
config_file_data = config_data.file_dict[SUBJECT]

blockspath = BT_DB['BASE_DIR']
...
MRfile = BT_DB_MRI['MRI_FILE']#'/home/acasamitjana/Data/change_orientation/P57_16_scanned_20170607/averageWithReg.reoriented.bfcorr.masked.uint8.rasoriented.nii.gz'
...
II, JJ, KK = np.meshgrid(np.arange(0, siz[0]), np.arange(0, siz[1]), np.arange(0, siz[2]), indexing='ij')


rasMosaic = np.dot(new_vox2ras0,
                   np.concatenate((II.reshape(-1, 1), JJ.reshape(-1, 1), KK.reshape(-1, 1), np.ones((num_voxels, 1))), axis=1).T)
...
if not exists(join(outputdir, 'upsampledMRI.nii.gz')) or not exists(join(outputdir, 'upsampledMRIMask.nii.gz')) \
    or not exists(join(outputdir, 'upsampledMRICR.nii.gz')) or not exists(join(outputdir, 'upsampledMRIMaskCLL.nii.gz')) \
    ...
    points = (np.arange(0, mri.shape[0]), np.arange(0, mri.shape[1]), np.arange(0, mri.shape[2]))
    ...
    if not exists(join(outputdir, 'upsampledMRI.nii.gz')):
        data = interpn(points, mri, xi=xi, method='linear')
    ...
    if not exists(join(outputdir, 'upsampledMRIMask.nii.gz')):
        ...
        data = interpn(points, mri_mask, xi=xi, method='linear')
        new_mri = np.zeros((num_voxels,))
        new_mri[ok] = data
        new_mri[np.isnan(new_mri)] = 0
...
for d in block_list:
    print(d, end='\n', flush=True)
    if not exists(join(blockspath, d, 'MRI_images.nii.gz')):
        continue

    proxy = nib.load(join(blockspath, d, 'MRI_images.nii.gz'))
    vox2ras0 = np.load(join(blockspath, d, 'vox2ras0.npy'))
    ...
    #LFB
    proxy = nib.load(join(blockspath, d, 'LFB_images.nii.gz'))
    mri = np.asarray(proxy.dataobj)
    data = interpn(points, mri, xi=xi, method='linear')
    data_prob = interpn(points, mri>0, xi=xi, method='linear')
    ...
    #HE
    proxy = nib.load(join(blockspath, d, 'HE_images.nii.gz'))
    mri = np.asarray(proxy.dataobj)
    data = interpn(points, mri, xi=xi, method='linear')
    data_prob = interpn(points, (mri>0), xi=xi, method='linear')
...
nib.save(img, join(outputdir, 'LFB.nii.gz'))
...
nib.save(img, join(outputdir, 'HE.nii.gz'))
