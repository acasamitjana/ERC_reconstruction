...
files = ['LABELS_cortical','LABELS', 'LFB_IMAGE', 'HE_IMAGE', 'LFB', 'HE']
...
for d in block_list:
    ...
    if exists(join(blockspath, d, 'LFB_IMAGE.nii.gz')):
        proxy = nib.load(join(blockspath, d, 'LFB_IMAGE.nii.gz'))
        mri = np.asarray(proxy.dataobj)
        my_interpolating_function = rgi(points, mri)
        data = my_interpolating_function(xi)

        data = data.astype(np.float32)
        lfb_num3[ok] += data * data_prob[..., np.newaxis]
        del data

        del mri, data_prob

    #HE
    proxy = nib.load(join(blockspath, d, 'HE.nii.gz'))
    mri = np.asarray(proxy.dataobj)
    ...
    if exists(join(blockspath, d, 'HE_IMAGE.nii.gz')):
        proxy = nib.load(join(blockspath, d, 'HE_IMAGE.nii.gz'))
        mri = np.asarray(proxy.dataobj)
        my_interpolating_function = rgi(points, mri)
        data = my_interpolating_function(xi)

        data = data.astype(np.float32)
        he_num3[ok] += data * data_prob[..., np.newaxis]
...
lfb_volume = np.zeros(siz + (3,))
for it_c in range(3):
    lfb_volume[..., it_c] = (lfb_num3[..., it_c] / (lfb_den + 1e-5)).reshape(siz)
img = nib.Nifti1Image(lfb_volume.astype('float32'), new_vox2ras0)
nib.save(img, join(outputdir, 'LFBc.nii.gz'))
...
he_volume = np.zeros(siz + (3,))
for it_c in range(3):
    he_volume[..., it_c] = (he_num3[..., it_c] / (he_den + 1e-5)).reshape(siz)
img = nib.Nifti1Image(he_volume.astype('float32'), new_vox2ras0)
nib.save(img, join(outputdir, 'HEc.nii.gz'))
del he_volume
