...
class SSIM_Loss(_Loss):
    ...
    def forward(self, prediction, target, mask=None, weight=None, *args, **kwargs):
        if mask is None and weight is None:
            ...
            if torch.sum(mask) == 0:
                return torch.tensor(0).float().to(mask.device)
        ...
        if self.reduction == 'mean':
            ...
            # norm_factor = torch.sum(torch.sum(torch.sum(mask, dim=-1), dim=-1), dim=-1, keepdim=True) + 1e-5
            # wk = 1 / norm_factor * torch.sum(torch.sum(mask, dim=-1), dim=-1)
            # res = 1 / norm_factor * torch.sum(torch.sum(res, dim=-1), dim=-1)
...
class GlobalNCC_Loss(_Loss):
    ...
    @NotImplementedError
    def _globalncc_loss(self, prediction, target):
        pass
...
class NCC_Loss(_Loss):
    ...
    def ncc(self, prediction, target):
        ...
        # pdb.set_trace()
        # slices([cc[0, 0], cc2[0, 0], cross[0, 0]])
        # slices([prediction[0,0], target[0,0], prediction[0,0] - target[0,0], cc[0, 0], cc2[0, 0], cc3[0, 0]])
        # slices([prediction[0,0], target[0,0],  prediction[0,0] - target[0,0], cc[0, 0], cross[0, 0], I_var[0, 0], J_var[0, 0]])
        # slices([prediction[0,0], target[0,0],  prediction[0,0] - target[0,0], cc[0, 0], cc2[0, 0]  ])
        # slices([I_var[0,0], J_var[0,0], cross[0,0], cc[0,0]])
    ...
    def forward(self, prediction, target, mask=None, *args, **kwargs):
        ...
        if mask is None:
            ...
            norm_factor = 1 / (torch.sum(mask) + 1e-5)
...
class SemiDice_Loss(_Loss):
    ...
    def forward(self, prediction, target, eps = 0.0000001, *args, **kwargs):
        """Dice loss.
        ...
        label_mask = target.sum(dim=list(range(2, ndims + 2))) > 0

        top = 2 * (target * prediction).sum(dim=vol_axes)
        bottom = torch.clamp((target + prediction).sum(dim=vol_axes), min=1e-5)

        dice = (top / bottom).sum(dim=1) / (label_mask.sum(dim=1)+1e-5)

        return - (dice * ((label_mask.sum(dim=1))>0)).sum()/((label_mask.sum(dim=1))>0).sum()


class CrossEntropy_Loss(_Loss):
    def __init__(self, name=None, class_weights=None, ignore_index=-100, reduction='mean', **kwargs):
        if name is None:
            name = 'cross_entropy'
        super().__init__(name=name)
...
class NLL_Loss(_Loss):
    def __init__(self, name=None, class_weights=None, ignore_index=-100, reduction='mean', **kwargs):
        if name is None:
            name = 'cross_entropy'
        super().__init__(name=name)
        self.loss = nn.NLLLoss(weight=class_weights, ignore_index=ignore_index, reduction=reduction)
    ...
    def forward(self, prediction, target, **kwargs):
        ...
        Arguments:
            ...
            - target: torch variable of size (batch_size, num_classes, d1, d2, ..., dN) representing  the ground-truth values
        """
        prediction = torch.log(prediction + 1e-5)
        prediction = torch.clamp(prediction, -10000, 0)
        _, labels = target.max(dim=1)
        return self.loss(prediction, labels)


class Overlap_Loss(_Loss):

    def __init__(self, name=None, alpha=1, beta=1):
        if name is None:
            name = 'overlap'
        super().__init__(name=name)
        self.alpha = alpha
    ...
    def forward(self, pred, targ, mask=None):
        ...
        aux = pred - targ
        w = -self.beta * torch.ones_like(aux)
        w[aux>0] = self.alpha
        aux = aux*w
        if mask is not None:
            aux = aux*mask
            return torch.sum(aux)/torch.sum(mask)

        else:
            return torch.mean(aux)

class Scaling_Loss(_Loss):

    def __init__(self, name=None, refResolution=1):
        if name is None:
            name = 'scale'
        super().__init__(name=name)
    ...
    def forward(self, header):
        ...
        if isinstance(header, list):
            ...
            loss = 0
            for h in header:
                naturalScale = torch.abs(torch.det(h)) ** (1 / 3) / self.refResolution
                logScale = torch.log(naturalScale)
                loss += logScale ** 2
            loss = loss/num_header

        else:
            naturalScale = torch.abs(torch.det(header) ** (1 / 3)) / self.refResolution
            logScale = torch.log(naturalScale)
            loss = logScale**2
        return loss
...
class Grad_Loss(_Loss):
    ...
    def _grad3d(self, prediction, **kwargs):
    ...
    def _grad2d(self, prediction, **kwargs):
        dy = torch.abs(prediction[:, :, 1:] - prediction[:, :, :-1])
        dx = torch.abs(prediction[:, :, :, 1:] - prediction[:, :, :, :-1])
        ...
        if 'mask' in kwargs:
            ...
            norm_factor = torch.sum(torch.sum(torch.sum(kwargs['mask'], dim=-1), dim=-1), dim=-1)
            d = 1/norm_factor*(torch.sum(torch.sum(torch.sum(dx, dim=-1), dim=-1), dim=-1) +
                               torch.sum(torch.sum(torch.sum(dy, dim=-1), dim=-1), dim=-1))
            d = torch.mean(d)
        else:
    ...
    def forward(self, prediction, mask=None, **kwargs):
        if mask is not None:
            ...
            prediction = prediction * (torch.sum(mask, (1 + np.arange(self.dim + 1)).tolist(), keepdims=True) > 0)

        if self.dim == 2:
            loss = self._grad2d(prediction, **kwargs)
        else:
            loss = self._grad3d(prediction, **kwargs)
...
DICT_LOSSES = {
    ...
    'NCC': NCC_Loss,
    'GlobalNCC': GlobalNCC_Loss,
    'MI': NMI_Loss,

    'BCE': BCE_Loss,
    'Dice': DiceLoss,
    'SemiDice': SemiDice_Loss,
